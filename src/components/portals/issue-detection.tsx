...
type CrisisTypeFilter = "flood" | "fire" | "medical" | "poverty" | "education" | "livelihood" | "all"

const CRISIS_TYPES = ["flood", "fire", "medical", "poverty", "education", "livelihood"] as const
...
export default function CrisisIssueDetection() {
  ...
  // Filters state
  const [selectedTypes, setSelectedTypes] = useState<CrisisTypeFilter[]>(["all"])
  const [severityMin, setSeverityMin] = useState(0)
  const [selectedDivisions, setSelectedDivisions] = useState<string[]>([])
  const [sortBy, setSortBy] = useState<"trending" | "newest" | "severity" | "funding-gap">("trending")
  ...
  const filteredCrises = useMemo(() => {
    ...
    if (!selectedTypes.includes("all")) {
      result = result.filter((c) => selectedTypes.includes(c.type as CrisisTypeFilter))
    }
    ...
    if (selectedDivisions.length > 0) {
      result = result.filter((c) => selectedDivisions.includes(c.location.division))
    }
    ...
    if (sortBy === "trending") {
      ...
      result.sort((a, b) => b.fundingNeeded - b.fundingReceived - (a.fundingNeeded - a.fundingReceived))
    }
  ...
  const toggleDivision = (division: string) => {
    setSelectedDivisions((prev) => (prev.includes(division) ? prev.filter((d) => d !== division) : [...prev, division]))
  }

  const fundingStats = useMemo(() => {
    const total = filteredCrises.reduce((sum, c) => sum + c.fundingNeeded, 0)
    const received = filteredCrises.reduce((sum, c) => sum + c.fundingReceived, 0)
    const affected = filteredCrises.reduce((sum, c) => sum + c.affectedPopulation, 0)
      ...
      <header className="border-b border-border/40 bg-background/80 backdrop-blur sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-3xl sm:text-4xl font-bold text-foreground">Crisis Detection Hub</h1>
              <p className="text-foreground/60 text-sm mt-1">
                Real-time AI monitoring • Satellite verification • Community validated
              </p>
            </div>
            <div className="flex flex-col sm:text-right">
              <div className="text-2xl font-bold text-primary">৳{(fundingStats.gap / 1000000).toFixed(1)}M</div>
          ...
          <div className="grid grid-cols-3 gap-3 mt-6 p-3 bg-card rounded-lg border border-border/50">
            <div className="text-center">
              <div className="text-lg font-semibold text-foreground">{filteredCrises.length}</div>
              <div className="text-xs text-foreground/60">Active Crises</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-semibold text-accent">{fundingStats.affected.toLocaleString()}</div>
              <div className="text-xs text-foreground/60">Affected People</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-semibold text-primary">
                {((fundingStats.received / fundingStats.total) * 100).toFixed(0)}%
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <aside className="lg:col-span-1">
            <div className="sticky top-24 space-y-6">
              ...
              {/* Crisis Type Filter */}
              <div className="bg-card border border-border/50 rounded-lg p-4">
                <h3 className="font-semibold text-foreground text-sm mb-3">Crisis Type</h3>
              ...
              {/* Severity Filter */}
              <div className="bg-card border border-border/50 rounded-lg p-4">
                <h3 className="font-semibold text-foreground text-sm mb-3">Min Severity</h3>
                <input
                  type="range"
                  min="0"
                  max="100"
                ...
                <div className="flex justify-between mt-2">
                  ...
                  <span className="text-sm font-medium text-primary">{severityMin}</span>
          ...
          <main className="lg:col-span-3">
            {filteredCrises.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-foreground/60">No crises match your filters</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredCrises.map((crisis) => (
                  <div key={crisis.id} className="group cursor-pointer" onClick={() => setSelectedCrisis(crisis.id)}>
                    <CrisisCard
                      crisis={crisis}
