...
export default function BeneficiaryDashboard() {
  ...
  const historicalApplications = allApplications.filter(app => 
    !app.appliedDate.startsWith('2025')
  )

  // Get current application (most recent ongoing)
  const currentApplication = currentYearApplications.find(app => 
    app.status === 'in-progress' || app.status === 'applied' || app.status === 'accepted'
  ...
  const paginatedApplications = historicalApplications.slice(startIndex, startIndex + itemsPerPage)

  const getStatusConfig = (status: string) => {
    const configs = {
      completed: { color: "bg-green-500 text-white", icon: CheckCircle, label: "Completed" },
      "in-progress": { color: "bg-blue-500 text-white", icon: Clock, label: "In Progress" },
      accepted: { color: "bg-purple-500 text-white", icon: TrendingUp, label: "Accepted" },
      rejected: { color: "bg-red-500 text-white", icon: XCircle, label: "Rejected" },
      applied: { color: "bg-yellow-500 text-white", icon: AlertCircle, label: "Applied" },
    }
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Manage all your applications - Current: {currentYearApplications.length} | Historical: {historicalApplications.length}
              </CardDescription>
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {allApplications.filter(a => a.status === 'completed').length}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">
              {allApplications.filter(a => a.status === 'in-progress' || a.status === 'applied' || a.status === 'accepted').length}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-emerald-600">
              {formatAmount(allApplications.reduce((sum, a) => sum + (a.amountReceived || 0), 0))}
            </div>
            <p className="text-xs text-muted-foreground mt-1">Lifetime assistance</p>
          </CardContent>
        </Card>
      </div>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>Your active application - {beneficiary.needCategory} assistance</CardDescription>
              </div>
              <Badge className={getStatusConfig(currentApplication.status).color}>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-medium capitalize">{beneficiary.needCategory}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground mb-1">Amount Requested</p>
                <p className="font-medium text-lg text-emerald-600">{formatAmount(beneficiary.amountRequested)}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground mb-1">Applied Date</p>
                <p className="font-medium">{new Date(beneficiary.appliedDate).toLocaleDateString()}</p>
              </div>
              <div>
            ...
            <div>
              ...
              <p className="text-sm bg-muted p-3 rounded-lg">{beneficiary.needDescription}</p>
            </div>

            {/* Application Timeline */}
            <ApplicationTimeline 
              beneficiary={beneficiary} 
              provider={beneficiary.allocatedProviderId ? getProviderForApplication(beneficiary.allocatedProviderId) : null}
            ...
            {currentYearApplications.length > 1 && (
              <div>
                <h4 className="font-semibold mb-3">Suggested Providers Based on Previous History (2025)</h4>
                <p className="text-xs text-muted-foreground mb-3">Based on your application history, the system has suggested these providers as good matches for your current needs</p>
                    ...
                    return (
                      <div key={idx} className="p-3 border rounded-lg flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium">{provider?.organizationName || 'Unknown Provider'}</p>
                            <p className="text-xs text-muted-foreground">
                              Matched: {new Date(app.appliedDate).toLocaleDateString()}
                            </p>
                          </div>
                        </div>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>Your application history from previous years</CardDescription>
              </div>
            </div>
          </CardHeader>
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="font-semibold">{provider?.organizationName || 'Unknown Provider'}</h4>
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm mb-3">
                      {app.responseDate && (
                        <div>
                          <p className="text-muted-foreground text-xs">Response Date</p>
                          <p className="font-medium">{new Date(app.responseDate).toLocaleDateString()}</p>
                        </div>
                      )}
                      {app.completionDate && (
                        <div>
                      ...
                      {app.amountReceived && (
                        <div>
                          ...
                          <p className="font-medium text-green-600">{formatAmount(app.amountReceived)}</p>
                        </div>
                      )}
                      {app.rating && (
                        <div>
                          ...
                          <div className="flex gap-1">
                            {Array.from({ length: 5 }).map((_, i) => (
                              <div
                                key={i}
                                className={`w-4 h-4 rounded ${i < app.rating! ? "bg-yellow-400" : "bg-gray-300"}`}
                              />
                            ))}
                          </div>
                        </div>
      ...
      {selectedApplication && (
        <Card className="fixed inset-4 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-2xl md:w-full z-50 shadow-2xl overflow-y-auto max-h-[90vh]">
          ...
          <CardContent className="p-6 space-y-4">
            {(() => {
              const provider = getProviderForApplication(selectedApplication.providerId)
              ...
              return (
                <>
                  <div className="flex items-center gap-3 pb-4 border-b">
                    ...
                    <div>
                      <h3 className="font-semibold text-lg">{provider?.organizationName}</h3>
                      <Badge className={statusConfig.color}>{statusConfig.label}</Badge>
                    </div>
                  </div>
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    {selectedApplication.responseDate && (
                      <div>
                        ...
                        <p className="font-medium">{new Date(selectedApplication.responseDate).toLocaleDateString()}</p>
                      </div>
                    )}
                    {selectedApplication.completionDate && (
                      <div>
                        <p className="text-sm text-muted-foreground mb-1">Completion Date</p>
                        <p className="font-medium">{new Date(selectedApplication.completionDate).toLocaleDateString()}</p>
                      </div>
                    )}
                    {selectedApplication.amountReceived && (
                      <div>
                        <p className="text-sm text-muted-foreground mb-1">Amount Received</p>
                        <p className="font-medium text-xl text-green-600">{formatAmount(selectedApplication.amountReceived)}</p>
                      </div>
                    )}
                  </div>
                    ...
                    <div className="p-4 bg-muted rounded-lg">
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <div className="flex gap-2 flex-wrap mt-2">
                          <Badge>Trust Score: {provider.trustScore}/100</Badge>
                          <Badge variant="outline">{provider.completionRate}% Completion</Badge>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {selectedApplication.rejectionReason && (
                    <div className="p-4 bg-red-50 dark:bg-red-950 border border-red-200 rounded-lg">
                      <p className="text-sm font-medium text-red-900 dark:text-red-100 mb-2">Rejection Reason</p>
                      <p className="text-sm text-red-700 dark:text-red-300">{selectedApplication.rejectionReason}</p>
                    </div>
                  )}

                  {selectedApplication.rating && (
                    <div className="p-4 bg-green-50 dark:bg-green-950 border border-green-200 rounded-lg">
                      <p className="text-sm font-medium text-green-900 dark:text-green-100 mb-2">Your Rating</p>
                      <div className="flex gap-1 mb-2">
                        {Array.from({ length: 5 }).map((_, i) => (
                          <div
                            key={i}
                            className={`w-6 h-6 rounded ${i < selectedApplication.rating! ? "bg-yellow-400" : "bg-gray-300"}`}
                          />
                        ))}
                      </div>
                      {selectedApplication.feedback && (
                        <p className="text-sm text-green-700 dark:text-green-300 italic">"{selectedApplication.feedback}"</p>
                      )}
                    </div>
                  )}
                </>
