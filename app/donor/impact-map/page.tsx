...
export default function ImpactMapPage() {
  ...
  useEffect(() => {
    ...
    const beneficiariesReached = userDonations.reduce(
      (sum, d) => sum + d.allocations.length,
      0
    );
    const crisisTypesSupported = new Set(
      userDonations.map(
        (d) => mockCrises.find((c) => c.id === d.crisisId)?.type
      )
    ).size;
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        {stats && (
          <div className="space-y-6">
            ...
            {/* Bangladesh Heatmap - Enhanced with Real-Time Data */}
            <BangladeshMap />
            ...
            <Card className="p-6">
              ...
              <div className="space-y-4">
                {stats.donations.map((donation: any, idx: number) => {
                  const crisis = mockCrises.find(
                    (c) => c.id === donation.crisisId
                  );
                  const beneficiaries = mockBeneficiaries.filter((b) =>
                    donation.allocations.some(
                      (a: any) => a.beneficiaryId === b.id
                    )
                  );
